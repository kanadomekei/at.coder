...
fn monadic_result_patterns() {
    ...
    println!("complex_calculation(100.0, 4.0) = {:?}", complex_calculation(100.0, 4.0));
    println!("complex_calculation(100.0, 0.0) = {:?}", complex_calculation(100.0, 0.0));
    println!("complex_calculation(-100.0, 4.0) = {:?}", complex_calculation(-100.0, 4.0));
    ...
    println!("multiple_operations([4.0, 9.0, 16.0]) = {:?}", 
...
fn practical_functional_example() {
    ...
    let orders = vec![
        Order { id: 1, user_id: 1, amount: 150.0, items: vec!["Laptop".to_string(), "Mouse".to_string()] },
        Order { id: 2, user_id: 2, amount: 75.0, items: vec!["Keyboard".to_string()] },
        Order { id: 3, user_id: 1, amount: 300.0, items: vec!["Monitor".to_string()] },
        Order { id: 4, user_id: 3, amount: 50.0, items: vec!["Cable".to_string()] },
    ];
    
    // 関数型的なデータ処理
    
    // 1. ユーザーごとの総注文額
    let user_totals: HashMap<u32, f64> = orders
        .iter()
    ...
    let high_value_users: Vec<(User, f64)> = users
        .iter()
        .filter_map(|user| {
            user_totals.get(&user.id).map(|&total| (user.clone(), total))
        })
    ...
    let item_counts: HashMap<String, usize> = all_items
        .iter()
...
mod tests {
    ...
    fn test_result_chaining() {
        ...
        let error_result = safe_divide(100.0, 0.0)
            .and_then(|x| safe_sqrt(x));
