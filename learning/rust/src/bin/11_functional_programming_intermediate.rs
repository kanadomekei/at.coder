...
fn closure_capture() {
    ...
    let process_data = move || data.iter().map(|x| x * 2).collect::<Vec<i32>>();
...
fn iterator_basics() {
    ...
    // チェーンメソッド
    let result: Vec<i32> = numbers.iter().map(|x| x * 2).filter(|&x| x > 4).collect();
...
fn fold_operations() {
    ...
    let word_stats = text.split_whitespace().fold(
        (0, 0), // (単語数, 文字数)
        |(word_count, char_count), word| (word_count + 1, char_count + word.len()),
    );
...
fn data_processing_pipeline() {
    ...
    // 高額商品の分析
    let high_value_sales: Vec<&Sale> = sales.iter().filter(|sale| sale.amount > 50.0).collect();
